...
export async function github<T>(
  path: string,
  init?: RequestInit,
): Promise<{ data: T; rate?: Rate; more: boolean }> {
  ...
  if (!response.ok) {
    ...
    throw new GitHubError(
      limited
        ? "GitHub request limit reached. Wait for the reset time before refreshing."
        : response.status === 404
          ? "The requested GitHub developer or repository was not found."
          : response.status === 401
            ? "The server GitHub token is invalid. Contact the site administrator."
            : response.status === 403
              ? "GitHub temporarily restricted this request. Please try again later."
              : response.status === 422
                ? "GitHub could not process this search. Check your query and filters."
